...
class Enemy:
    def __init__(self, x_grid, y_grid, enemy_id):
        ...
        self.rect = pygame.Rect(ROOM_OFFSET_X + x_grid*TILE_SIZE+5, ROOM_OFFSET_Y + y_grid*TILE_SIZE+5, 30, 30)
...
def draw_room(room_data):
    ...
    for y in range(16):
        for x in range(16):
            ...
            if layout[y][x] == 1: pygame.draw.rect(screen, LIGHT_GRAY, r.inflate(-4, -4))
    rx, ry = current_room
    for dx, dy, side in [(0,-1,"top"), (0,1,"bottom"), (-1,0,"left"), (1,0,"right")]:
        if (rx+dx, ry+dy) in game_map:
            if side == "top": pygame.draw.line(screen, BLUE, (ROOM_OFFSET_X+280, ROOM_OFFSET_Y), (ROOM_OFFSET_X+360, ROOM_OFFSET_Y), 10)
            elif side == "bottom": pygame.draw.line(screen, BLUE, (ROOM_OFFSET_X+280, ROOM_OFFSET_Y+640), (ROOM_OFFSET_X+360, ROOM_OFFSET_Y+640), 10)
            elif side == "left": pygame.draw.line(screen, BLUE, (ROOM_OFFSET_X, ROOM_OFFSET_Y+280), (ROOM_OFFSET_X, ROOM_OFFSET_Y+360), 10)
            elif side == "right": pygame.draw.line(screen, BLUE, (ROOM_OFFSET_X+640, ROOM_OFFSET_Y+280), (ROOM_OFFSET_X+640, ROOM_OFFSET_Y+360), 10)
...
def start_new_game():
    ...
    player_hp = 3; player_bullets = []; player_inv_timer = 0; scene = 1
...
while running:
    ...
    if scene == 0:
        screen.fill(WHITE)
        btn_s = pygame.Rect(SCREEN_WIDTH//2-120, SCREEN_HEIGHT//2-80, 240, 60)
        btn_q = pygame.Rect(SCREEN_WIDTH//2-120, SCREEN_HEIGHT//2+20, 240, 60)
        pygame.draw.rect(screen, GREEN, btn_s, border_radius=12); pygame.draw.rect(screen, RED, btn_q, border_radius=12)
        txt_s = menu_font.render("START", True, BLACK); txt_q = menu_font.render("QUIT", True, BLACK)
        screen.blit(txt_s, txt_s.get_rect(center=btn_s.center)); screen.blit(txt_q, txt_q.get_rect(center=btn_q.center))
        if event.type == pygame.MOUSEBUTTONDOWN:
            if btn_s.collidepoint(event.pos): start_new_game()
            if btn_q.collidepoint(event.pos): running = False
        ...
        if player_inv_timer % 10 < 5: pygame.draw.rect(screen, (0, 200, 255), p_rect)
